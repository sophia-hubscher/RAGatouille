...
with open('output.tsv', 'w', buffering=1) as file:
    tsv_writer = csv.writer(file, delimiter='\t')
    original_stdout = sys.stdout
    sys.stdout = open(os.devnull, 'w')
    for line in tqdm.tqdm(data, desc="Processing lines"):
        question, answer, ground_truth = line
        our_response = rag_system.get_response(question)
        if our_response.get('error', -1) != -1:
            original_stdout.write(f"ERROR {question}")
            continue
        our_answer = our_response.get('answer', -1)
        our_sources = our_response.get('sources', -1)
        if our_answer == -1 or our_sources == -1:
            original_stdout.write(f"ERROR {question}")
            continue
        row = [question, answer, ground_truth, our_answer, our_sources]
        row.append(str(accuracy(question, our_answer, answer)))
        row.append(str(relevance(question, our_answer, ground_truth)))
        row.append(str(groundedness(question, our_answer, our_sources)))
        tsv_writer.writerow(row)

    tsv_writer.writerow(["question", "answer", "ground_truth", "our_answer", "our_sources"])
    sys.stdout.close()
    sys.stdout = original_stdout


'''
response = dict(rag_system.get_response("What are ISO NE's Bilateral Transactions"))
print("\n \n \n \n \n \n \n \n \n \n")
